...
from src.main.business.template.google_calendar_template_builder_impl import GoogleCalendarTemplateBuilderImpl
from src.main.infra.utils.log_utils import log
from src.main.infra.security.google_credential_manager import GoogleCredentialManager
...
class GoogleCalendarNotificationProviderImpl(NotificationProviderI):
    ...
    def notify(self, events: list[EventDTO], user: UserDTO) -> None:
        ...
        new_event_models: list[EventDTO] = self._remove_existing_calendar_events(event_models=events)
    ...
    def _remove_existing_calendar_events(self, event_models: list[EventDTO]) -> list[EventDTO]:
