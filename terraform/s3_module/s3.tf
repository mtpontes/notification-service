...
resource "aws_s3_object" "publisher_lambda_zip_object" {
  ...
  source       = "${path.module}/${var.publisher_service_lambda_file_zip_name}.zip"
  etag         = filemd5("${path.module}/${var.publisher_service_lambda_file_zip_name}.zip")
...
resource "aws_s3_object" "dispatcher_lambda_zip_object" {
  ...
  source       = "${path.module}/${var.dispatcher_service_lambda_file_zip_name}.zip"
  etag         = filemd5("${path.module}/${var.dispatcher_service_lambda_file_zip_name}.zip")
  content_type = "application/zip"
}
